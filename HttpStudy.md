...
一台Web服务器可搭建多个独立域名的Web网站，也可作为通信路径上的中转服务器提升效率

### 5.1 使用单台虚拟主机实现多个域名

HTTP/1.1规范允许一台HTTP服务器搭建多个Web站点。利用虚拟主机的功能，可以以每位客户持有的域名运行各自不同的网站。

在相同的IP地址下，由于虚拟主机可以寄存多个不同主机名和域名的Web网站，因此在发送HTTP请求时，必须在Host首部内完整指定主机名或域名URI。

### 5.2 通信数据转发程序：代理、网关、隧道

将请求转发给通信路线上的下一站服务器，并且能接收从那台服务器发送的响应再转发给客户端。

- 代理：是一种有转发功能的应用程序，它扮演了位于服务器和客户端”中间人“的角色，接收由客户端发送的请求，并转发给服务器，同时也接收服务器端返回的响应并转发给客户端

  代理服务器的基本行为就是接收客户端发送的请求后，转发给其他服务器。代理不会改变请求URI，会直接发送给前方持有资源的目标服务器。每次通过代理服务器转发请求或响应时，会追加Via首部信息。

  使用代理的理由：利用网络缓存技术减少网络带宽的流量，组织内部针对特定网站的访问控制，以获取访问日志为主要目的等

  代理按照两种基准分类：是否使用缓存；是否会修改报文

  - 缓存代理：预先将资源的副本保存在代理服务器上，再次接收对相同资源的请求时，就可以不从源服务器哪里获取资源，而将之前缓存的资源作为响应返回
  - 透明代理：不对报文做任何加工的代理类型。

- 网关：是转发其他服务器通信数据的服务器，解说从客户端发送来的请求时，他就像自己拥有资源的服务器一样对请求进行处理。有时客户端都不会察觉，自己的通信目标是一个网关。

  与代理相似，但能够使通信线路上的服务器提供非HTTP协议服务。

- 隧道：在相隔甚远的客户端和服务器两者之间进行中转，并保持双方通信连接的应用程序。

  按照要求建立一条与其他服务器的通信线路，届时使用SSL等加密手段进行通信。目的是确保客户端与服务器进行安全的通信。

### 5.3 保存资源的缓存

缓存：指代理服务器或本地客户端磁盘内保存的资源副本。利用缓存可以减少对源服务器的访问，节省流量和通信时间。

缓存服务器是代理服务器的一种，归类在缓存代理类型中。当代理转发从服务器返回的响应时，代理服务器会保存一份资源的副本。

#### 5.3.1 缓存的有效期限

因客户端的要求、缓存的有效期等因素，代理会向源服务器确认资源的有效性。弱判断缓存失效，缓存服务器将会再次从源服务器上获取新资源

#### 5.3.2 客户端的缓存

缓存可以存在于客户端浏览器中。成为临时网络文件。如果浏览器缓存有效，则不用向服务器请求相同资源，直接从本地磁盘读取。

当缓存过期后，再向服务器确认资源有效性。失效则再次请求新资源
